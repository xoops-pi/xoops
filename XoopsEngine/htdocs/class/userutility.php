 ...
 * @version         $Id: userutility.php 1982 2008-08-24 13:39:55Z phppp $
 */
...
class XoopsUserUtility
{
    ...
    function validate(/*$uname, $email, $pass = null, $vpass = null*/)
    {
        global $xoopsConfigUser, $xoopsDB, $xoopsUser;
        $args = func_get_args();
        ...
        $sql = "SELECT COUNT(*) FROM `" . $xoopsDB->prefix('users') . "` WHERE `uname` = " . $xoopsDB->quote(addslashes($uname)) .
                ( ($uid > 0) ? " AND `uid` <> {$uid}" : "" );
        ...
        $sql = "SELECT COUNT(*) FROM `" . $xoopsDB->prefix('users') . "` WHERE `email` = " . $xoopsDB->quote(addslashes($email)) .
                ( ($uid > 0) ? " AND `uid` <> {$uid}" : "" );
        $result = $xoopsDB->query($sql);
        list($count) = $xoopsDB->fetchRow($result);
        if ($count > 0) {
