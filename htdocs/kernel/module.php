...
class XoopsModule extends XoopsObject
{
    ...
    public function __construct($row = null)
    {
        ...
        if (isset($row)) {
            ...
            $this->row = XOOPS::getModel("module")->createRow();
        }
    ...
    {
        $this->modinfo = Xoops_Module_Info::load($dirname);
        return true;
...
class XoopsModuleHandler extends XoopsObjectHandler
{
    ...
    {
        trigger_error(__METHOD__ . " is deprecated, please use XOOPS::getModel('module')", E_USER_DEPRECATED);
    ...
    function &get($id)
    {
        ...
        if ($id > 0) {
            if (!empty($_cachedModule_mid[$id])) {
                ...
                if (!$row = XOOPS::getModel("module")->findRow($id)) {
                    return $module;
                }
                $module = new XoopsModule($row);
                $_cachedModule_mid[$id] = $module;
                $_cachedModule_dirname[$module->getVar('dirname')] = $module;
                return $module;

                /*
                $sql = 'SELECT * FROM '.$this->db->prefix($this->table).' WHERE mid = '.$id;
    ...
    function &getByDirname($dirname)
    {
        ...
        if (!empty($_cachedModule_dirname[$dirname])) {
            ...
            $module = false;
            if (!$row = XOOPS::getModel("module")->load($dirname)) {
                return $module;
            }
            $module = new XoopsModule($row);
    ...
    {
        trigger_error(__METHOD__ . " is deprecated, please use XOOPS::getModel('module')", E_USER_ERROR);
        return array();
