<?php
// $Id: configitem.php 1947 2008-08-08 10:16:50Z phppp $
//  ------------------------------------------------------------------------ //
...
class XoopsConfigItemHandler extends XoopsObjectHandler
{
    ...
    function &get($id)
    {
        ...
        $id = intval($id);
        if ($id > 0) {
            $sql = 'SELECT * FROM '.$this->db->prefix('config', 'core').' WHERE conf_id='.$id;
    ...
    function delete(&$config)
    {
        ...
        */
        ...
        $sql = sprintf("DELETE FROM %s WHERE conf_id = %u", $this->db->prefix('config', 'core'), $config->getVar('conf_id'));
        if (!$result = $this->db->query($sql)) {
            return false;
        }
        return true;
    ...
    function getObjects($criteria = null, $id_as_key = false)
    {
        $ret = array();
        $limit = $start = 0;
        $sql = 'SELECT * FROM '.$this->db->prefix('config', 'core');
